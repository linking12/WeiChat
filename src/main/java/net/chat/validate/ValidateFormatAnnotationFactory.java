...
public class ValidateFormatAnnotationFactory implements AnnotationFormatterFactory<ValidateFormat> {
	...
	public Parser<?> getParser(ValidateFormat annotation, Class<?> fieldType) {
		return new MaskFormatter(annotation.value());
	}

	public Printer<?> getPrinter(ValidateFormat annotation, Class<?> fieldType) {
		return new MaskFormatter(annotation.value());
	}
